...
**Caso 1: Guía para instalar Ansible en Linux (El documento inicial de este repo.)**
...
### Por qué usar este tipo de estructuras es beneficiosa

1. **Memoria Externa:** No se tiene que recordar si decide usar `WIN` o `WINDOWS` para las etiquetas. Se mira el documento `_meta/GOVERNANCE.md` y sigue el estándar (consensuado por el grupo).
2. **Escalabilidad:** Si mañana se necesita documentar **"Terraform"**, simplemente se añade `TF` a la tabla de herramientas en este documento y ya se ha actualizado el estándar.
3. **Profesionalidad:** Tener una carpeta `_meta` con reglas de juego demuestra se se es un profesional o grupo de Ingenieros gestionando una base de conocimiento.
